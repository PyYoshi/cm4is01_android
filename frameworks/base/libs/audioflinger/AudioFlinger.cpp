...
#endif
{
    ...
    mAudioHardware = AudioHardwareWrapper::create();
...
status_t AudioFlinger::setMasterMute(bool muted)
{
    ...
    mAudioHardware->setMasterMute(muted);
...
status_t AudioFlinger::setStreamVolume(int stream, float value, int output)
{
    ...
    mAudioHardware->setStreamVolume(stream, value);
...
status_t AudioFlinger::setStreamMute(int stream, bool muted)
{
    ...
    mAudioHardware->setStreamMute(stream, muted);
...
{
    //FIXME: use mAudioHardware->isStreamActive(stream);
...
AudioFlinger::PlaybackThread::PlaybackThread(const sp<AudioFlinger>& audioFlinger, AudioStreamOutWrapper* output, int id)
        ...
        status_t *status)
{
    ...
    if (mType == DIRECT) {
        if (sampleRate != mSampleRate || format != mFormat || channelCount != mChannelCount) {
            ...
            LOGE("createTrack_l() current parameter:  sampleRate %d format %d, channelCount %d", mSampleRate, mFormat, mChannelCount);
            lStatus = BAD_VALUE;
            goto Exit;
        }
    } else {
...
{
    status_t status = mOutput->Open();
    if (status != NO_ERROR)
    {
        LOGE("mOutput->Open(): failed (%d)", status);
        return status;
    }
	...
	int stream = track->type();
        mAudioFlinger->mAudioHardware->setStreamMute(stream, streamMute(stream));
        mAudioFlinger->mAudioHardware->setStreamType(stream);
...
void AudioFlinger::PlaybackThread::destroyTrack_l(const sp<Track>& track)
{
    ...
    if (mActiveTracks.indexOf(track) < 0) {
        mAudioFlinger->mAudioHardware->setStreamType(AudioSystem::DEFAULT);
...
AudioFlinger::MixerThread::MixerThread(const sp<AudioFlinger>& audioFlinger, AudioStreamOutWrapper* output, int id)
...
// ----------------------------------------------------------------------------
AudioFlinger::DirectOutputThread::DirectOutputThread(const sp<AudioFlinger>& audioFlinger, AudioStreamOutWrapper* output, int id)
    :   PlaybackThread(audioFlinger, output, id),
...
AudioFlinger::RecordThread::RecordThread(const sp<AudioFlinger>& audioFlinger, AudioStreamInWrapper *input, uint32_t sampleRate, uint32_t channels, int id) :
    ThreadBase(audioFlinger, id),
                                ...
                                uint32_t flags)
{
    ...
    AudioStreamOutWrapper *output = mAudioHardware->openOutputStream(*pDevices,
                                                             (int *)&format,
                                                             &channels,
                                                             &samplingRate,
                                                             &status);
                                ...
                                uint32_t acoustics)
{
    ...
    AudioStreamInWrapper *input = mAudioHardware->openInputStream(*pDevices,
                                                             (int *)&format,
                                                             &channels,
                                                             &samplingRate,
                                                             &status,
